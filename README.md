...
- [x] create `nick_t` and scalar operations (both directions)
- [x] create `nick_t` unary operation `log`
...
- [x] update `nick_t` creation interface, eliminating any reference to constant matricies. Should appear like `nick_t(xdim,ydim).ones()` or `nick_t(xdim,ydim).scalar(10)` or `nick_t(xdim,ydim).eye()`
...
- [x] convenience function `grad_ball grad(nick_t output)` so that syntax is: `nick_t z = x + y; grad_ball gb = grad(z)`
